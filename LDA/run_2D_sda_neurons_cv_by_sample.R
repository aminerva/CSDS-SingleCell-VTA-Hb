...
output_prefix <- paste0(output_dir, "/2D_sda_", cluster)
...
n_folds <- length(unique(sampleIDs))

message("Running ", n_folds, "-fold cross validation SDA with true labels...")
unshuffled_result <- cross_validate_2D_sda(
  data_t = data_t,
  labels = labels,
...
shuffled_result <- cross_validate_2D_sda(
  data_t = data_t,
  labels = labels,
  genes = variable_genes,
  sample_ids = sampleIDs,
...
# Save the lambda used in each fold for inspection
write.csv(data.frame(Fold = 1:n_folds, Lambda = unshuffled_result$lambdas),
          file = paste0(output_prefix, "_unshuffled_lambdas.csv"), row.names = FALSE)

write.csv(data.frame(Fold = 1:n_folds, Lambda = shuffled_result$lambdas),
          file = paste0(output_prefix, "_shuffled_lambdas.csv"), row.names = FALSE)

# Also save accuracies
...
message("Now training on all data to visualize full 2D SDA projection")

full_sda_projection <- run_2D_sda_projection(
  data_t = data_t,
  labels = labels,
  genes = variable_genes,
  shuffle = FALSE)

saveRDS(full_sda_projection$sda_df, paste0(output_prefix, "_full_training_projection.rds"))

#########################################

message("Extracting gene contributions to LD1 and LD2")
gene_contributions <- as.data.frame(t(full_sda_projection$sda_model$beta[1:2, ]))
colnames(gene_contributions) <- c("LD1", "LD2")
gene_contributions$Gene <- rownames(gene_contributions)
